 ...
 *
 * Emits BOTH the mega-menu HTML (wrapped in .sge-mm-desktop, visible >=992px) and a
 * legacy-styled fallback (wrapped in .sge-mm-mobile, visible <992px). The mobile fallback
 * is generated by re-running wp_nav_menu() with bypass_sge_mm=true so core produces
 * untouched markup, but with the menu source overridden to the plugin's configured
 * default source — that way mobile shows the SAME items as the desktop mega menu and
 * updates in one place flow to both. All other args (container_id, menu_class,
 * items_wrap, walker, etc.) are preserved so the theme's mobile CSS still matches. */
function sge_mm_short_circuit_nav_menu( $output, $args ) {
	...
	// Override the menu source so the mobile fallback mirrors the desktop mega menu's items.
	// We touch only the menu/location keys — container, menu_class, items_wrap, etc. stay
	// intact so legacy mobile CSS (e.g. `.mobile-menu .menu`, `#side-menu`) still matches.
	$source = sge_mm_get_default_source();
	if ( ! empty( $source['menu_id'] ) ) {
		$orig_args['menu']           = (int) $source['menu_id'];
		$orig_args['theme_location'] = '';
	} elseif ( ! empty( $source['menu'] ) ) {
		$orig_args['menu']           = $source['menu'];
		$orig_args['theme_location'] = '';
	} elseif ( ! empty( $source['location'] ) ) {
		$orig_args['menu']           = '';
		$orig_args['theme_location'] = $source['location'];
	}
